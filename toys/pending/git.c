...
static void read_index(struct IndexV2 *i){
  ...
  if (access(".git/object/pack/temp.idx", F_OK)==0) {//TODO: not used yet as index is not persisted yet
    fpi=fopen(".git/object/pack/temp.idx","rb");//persistance needed for other git commands (not clone)
    printf("read header\n");
...
static char *l;//for saving the insertation position
...
long set_object(struct IndexV2 *idx,int type, char *o, uint32_t count, uint32_t ofs){
 ...
 // }
  ...
  idx->sha1=realloc(idx->sha1,(idx->fot[255]+1)*20*sizeof(char));//Did not fix the TODO yet, because set_object could be reused for other command im mem mgmt is here
...
{
  if (mkdir(name,0755)!=0){//For git clone actually only refs and object/pack needed
  ...
  *count=0;
  while(pos<dsize){
    ...
    //https://stackoverflow.com/a/14303988
      ...
      if (size==0) size=0x10000;//packfomat: size zero is automatically converted to 0x10000.https://github.com/git/git/blob/master/Documentation/technical/pack-format.txt#L133
//     printf("Size %d\n",size);
//      printf("Realloc\n");
      //printf("Memcpy %s %d %d\n",t,size,offset);
      ...
      pos+=j;
      printf("Pos\n");
    } else {//https://github.com/git/git/blob/master/Documentation/technical/pack-format.txt#L133
//      printf("Case 0\n");
